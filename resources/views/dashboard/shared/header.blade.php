  ...
  <header class="c-header c-header-light c-header-fixed c-header-with-subheader">
    <button class="c-header-toggler c-class-toggler d-lg-none mr-auto" type="button" data-target="#sidebar"
      ...
      <a class="c-header-brand d-sm-none" href="#">
        <img class="c-header-brand" src="{{ env('APP_URL') }}/assets/brand/coreui-base.svg" width="97" height="46" alt="CoreUI Logo">
      </a>
    ...
    <ul class="c-header-nav d-md-down-none">
      @if($user)
      <li class="c-header-nav-item px-3"><a class="c-header-nav-link" href="javascript:void(0)">Selamat Datang
          {{$user->name}} @if($user->sekolah_id) | {{$user->sekolah->nama}} @endif</a></li>
      @endif
    </ul>
    <ul class="c-header-nav ml-auto mr-4">
      <li class="c-header-nav-item dropdown"><a class="c-header-nav-link" data-toggle="dropdown" href="#" role="button"
          ...
          <div class="c-avatar">
            @if($user)
              @if($user->photo)
              <img class="c-avatar-img" src="{{ env('APP_URL') }}/storage/uploads/profile/50/{{$user->photo}}" alt="{{$user->name}}">
              @else
              <img class="c-avatar-img" src="{{ env('APP_URL') }}/assets/img/avatars/male-md.png" alt="{{$user->name}}">
              @endif
              <br>
            @else
    ...
    <div class="c-subheader px-3">
      <ol class="breadcrumb border-0 m-0">
        ...
        <?php $segments = ''; ?>
        @for($i = 1; $i <= count(Request::segments()); $i++) <?php $segments .= '/'. Request::segment($i); ?> @if($i <
          count(Request::segments())) <li class="breadcrumb-item">{{ Request::segment($i) }}</li>
          @else
